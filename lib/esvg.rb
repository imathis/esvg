...
module Esvg
  ...
  def config(options={})
    @config ||= begin
      ...
      config.merge!(CONFIG_RAILS) if options[:rails]
      config.merge(options)

      config[:css_path]  ||= config[:output_path]
      config[:html_path] ||= config[:output_path]
