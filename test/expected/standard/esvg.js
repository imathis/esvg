var esvg = {
  ...
  icon: function(name, class) {
    if (document.querySelector('#'+this.dasherize(name))) {
      return '<svg class="svg-icon '+name+' '+classname+'" '+this.dimensions(name)+'><use xlink:href="#'+name+'"/></svg>'
    } else {
      console.error('No icon named "'+name+'" exists at /Users/imathis/workspace/imathis/esvg/test/svg_icons')
    }
  },
